    ...
    async getOrders(entree_type, protein_type, chips_and_salsa, chips_and_queso, chips_and_guac, drink) {
        try {
            const response = await new Promise((resolve, reject) => {
                ...
                const query = `SELECT * FROM cabo_grill_sales WHERE (entree_type IN (?) AND protein IN (?) AND chips_and_salsa = ? AND chips_and_queso = ?
                  AND chips_and_guac = ? AND drink = ? AND sale_id > 1) ORDER BY sale_id DESC;`
                connection.query(query, [entree_type, protein_type, chips_and_salsa, chips_and_queso, chips_and_guac, drink], (err, results) => {
                    if (err)
                        reject(new Error(err.message));
                    else
                        resolve(results);
